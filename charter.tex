...
Este proyecto surge como una necesidad de la empresa {\empclientename} y ha sido propuesto por {\clientename} dentro del programa de vinculación con empresas de la {\degreename}.

{\empclientename} desarrolla y comercializa diversos dispositivos para la automatización de salas de cultivo. Uno de estos dispositivos es un sensor de suelo diseñado para medir la temperatura y humedad en invernaderos. Actualmente, este sensor utiliza el microcontrolador ESP32-C3 y transmite las mediciones vía Wi-Fi a un servidor local del usuario. Como resultado, cada sensor instalado en el invernadero debe estar dentro del alcance de la red Wi-Fi para funcionar correctamente.

El objetivo principal de este proyecto es aprovechar las capacidades Bluetooth del ESP32-C3 para desarrollar una solución de transmisión de datos basada en una red de sensores Bluetooth Mesh. Esta tecnología permitirá ampliar el rango de comunicación de los sensores, eliminando la necesidad de que cada uno esté dentro de un área específica de cobertura Wi-Fi. Además, se implementará un servidor web local para la configuración y monitoreo de la red, optimizando la gestión y operación del sistema para el usuario final.



\subsection{Red de sensores Bluetooth Mesh}
Una red de sensores Bluetooth Mesh es un sistema de comunicación inalámbrica en el que múltiples dispositivos, llamados nodos, se interconectan para formar una red descentralizada y de amplio alcance. A diferencia de las conexiones punto a punto tradicionales, en una red Mesh cada nodo puede retransmitir datos, lo que permite extender la cobertura de comunicación más allá del rango de un solo dispositivo y mejorar la confiabilidad del sistema.

En el contexto de sensores para invernaderos, una red Bluetooth Mesh permite que los datos recopilados por cada sensor sean transmitidos de manera eficiente a través de otros nodos hasta llegar a un punto central, sin necesidad de que todos los dispositivos estén dentro del alcance directo de una única fuente de comunicación, como una red Wi-Fi. Esto optimiza el consumo energético, reduce la infraestructura necesaria y facilita la escalabilidad del sistema.
...
utilizado es el ESP32-C3 de Espressif Systems.
Los datos recolectados por los sensores Bluetooth se envían al módulo central, el cual los
transmite al servidor mediante el protocolo MQTT para la comunicación cliente-servidor.
Se busca que el módulo central gestione la comunicación con los diferentes nodos sensores
del invernadero, permitiendo el monitoreo de variables y el estado de los actuadores
conectados, así como la ejecución de acciones programadas.
Asimismo, se desarrollará un servidor web que facilite el control y monitoreo del equipo
dentro de la red local, integrando a los distintos nodos BLE conectados.
...
\begin{itemize}
	\item Cliente: 
	\item Orientador: el... Dra. Ing. María Gómez es experto en la temática y va a ayudar con la definición de los requerimientos y el desarrollo del firmware del embebido.
...
Aprovechar las capacidades Bluetooth del microcontrolador ESP32-C3 para desarrollar una solución de transmisión de datos basada en una red de sensores Bluetooth Mesh para los sensores de suelo de {\clientename}. Esta tecnología permitirá ampliar el rango de comunicación de los sensores, eliminando la necesidad de que cada uno esté dentro de un área específica de cobertura Wi-Fi. Además, se implementará un servidor web local para la configuración y monitoreo de la red, optimizando la gestión y operación del sistema para el usuario final.
